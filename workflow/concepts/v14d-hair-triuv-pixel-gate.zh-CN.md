...
4. 用该 UV 对权威 hair_d 纹理做线性空间双线性采样，再乘唯一权威 V14D_HAIR_TINT，最后转换到显示字节比较。
...
- 正式目标必须使用 hair_d 同 UV 线性双线性采样，再应用 v14dAuthority.js 导出的 V14D_HAIR_TINT；不能使用槽位 targetMean。
...
- captureHairTriUv 只在显式 acceptance probe 下暴露；读取完成后恢复渲染循环和原有播放状态，默认生产入口不泄漏探针。
...
targetLinear(uv) = bilinearLinear(hair_d, uv) × V14D_HAIR_TINT

其中 hair_d 文件以 sRGB 字节读取并先转换到线性空间；比较值是目标线性色经 sRGB OETF 转回的显示字节。origMae 与 v1Mae 是屏幕样本在 RGB 三通道绝对误差平均后的显示字节 MAE；p95 是该像素误差的 95 百分位。
...
- HairA：materialId=24，samples=4185，coverage=0.037179，origMae=28.366，v1Mae=25.248，drop=0.1099，formalGate=true，triUV 与目标来源均为 HairA，p95 orig=71、v1=71。
- HairB：materialId=25，samples=15502，coverage=0.096946，origMae=47.589，v1Mae=35.744，drop=0.2489，formalGate=true，triUV 与目标来源均为 HairB，p95 orig=99.333、v1=89。
- 正式 visual-diff.json 不含 targetMean；逐槽目标由 triUV 同 UV 采样产生。

## 反例与负测

- 错槽/错目标：把 HairA 的 target/triUV 来源换成 HairB，同时把 HairB 换成 HairA。真实回放中 analyzer exit=1，negativeVerdict.status=rejected，HairA/HairB formalTargetGate 均为 false，analysisFailures=[]；失败保留在正式 failures 中，证明正式 Gate 确实阻断。
- wrongTint：只替换 HairA/HairB 的实际错误 tint 画布，其他 Gate 使用健康 V1。协议要求 analyzer exit=0、negativeVerdict.status=rejected、两槽正式 Gate 均为 false、failures=[]；任何配置缺失、样本不足或其他分析失败都不能算预期拒绝。
