...
import { isInjectedCodexContext, resolveCodexTaskTitle } from "../shared/codexPresentation.js";
...
import { launchNewCodexDesktopSession, launchNewCodexDesktopUnboundSession, } from "./codexDesktopLauncher.js";
import { buildDesktopPetSessionPayload, resolveCodexHome, scanRecentCodexSessionFiles, } from "./codexSessionFiles.js";
import { discoverLocalCodexSessions, discoverLocalClaudeSessions, discoverPetAppServerSessions, enrichAgentSessionRecords, } from "./agentSessionDiscovery.js";
...
import { buildActiveWorkspaceSummaries, buildPetMenuModel, normalizeMenuLanguage, normalizeNotificationProfile, normalizeCodexEnvMode, normalizeCodexLaunchTarget, normalizePetAgent, } from "./petMenuModel.js";
...
async function readAllLocalCodexSessions(limit = 50) {
    const codexHome = resolveCodexHome(process.env);
    const wslCodexHome = process.env.CODEX_WSL_HOME?.trim() || "\\\\wsl.localhost\\Ubuntu\\home\\ksg\\.codex";
...
async function readAllLocalClaudeSessions(limit = 50) {
    const claudeHome = resolveClaudeHome(process.env);
...
async function readAllPetAppServerSessions(limit = 50) {
...
async function readAllLocalAgentSessions(limit = 50) {
    ...
    const enrichedRecords = enrichAgentSessionRecords(records, processObservations, { scanCompleted: processScanCompleted });
...
function cacheMenuSessions(sessions) {
    lastMenuSessionsByPetId = new Map(sessions
        .map((session) => [sessionMenuKey(session), session])
