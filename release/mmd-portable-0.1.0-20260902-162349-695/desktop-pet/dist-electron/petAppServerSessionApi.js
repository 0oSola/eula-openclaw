...
function mapSession(value) {
    ...
    if (!id || !workspacePath || !createdAt || !lastActiveAt)
        return null;
...
export function createPetAppServerSessionScanner(options) {
    const apiBaseUrl = options.apiBaseUrl.replace(/\/+$/, "");
    const userId = options.userId.trim();
    const fetcher = options.fetcher ?? fetch;
    return async (limit) => {
        const boundedLimit = Math.max(1, Math.min(Math.floor(limit), 50));
        const response = await fetcher(`${apiBaseUrl}/codex/interactive/sessions?limit=${boundedLimit}`, { headers: { "x-user-id": userId } });
        if (!response.ok)
            throw new Error(`app-server sessions failed: ${response.status}`);
