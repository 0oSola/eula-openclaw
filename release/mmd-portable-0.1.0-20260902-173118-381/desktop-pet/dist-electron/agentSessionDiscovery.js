export const AGENT_SESSION_PROVIDERS = ["codex", "claude", "pet-app-server"];
export const AGENT_SESSION_RUNTIMES = ["desktop", "cli", "wsl", "app-server", "unknown"];
...
function isWslEvidence(summary) {
    return (summary.filePath.toLowerCase().includes("\\\\wsl.localhost\\") ||
...
function classifyRuntime(summary) {
    const originator = compact(summary.originator).toLowerCase();
    if (/desktop/.test(originator))
        return "desktop";
    if (compact(summary.source).toLowerCase() === "desktop")
        return "desktop";
    if (/cli|tui/.test(originator))
        return isWslEvidence(summary) ? "wsl" : "cli";
    if (compact(summary.source).toLowerCase() === "cli")
        return isWslEvidence(summary) ? "wsl" : "cli";
...
function sessionKeyFor(provider, sessionId) {
    return `local:${provider}:${sessionId}`;
}
function emptyReviewFacts() {
    return {
        failed_commands: [],
        changed_files: [],
        approvals: [],
...
function deriveState(status, lastActivityAt, nowMs, activeWindowMs = DEFAULT_ACTIVE_WINDOW_MS) {
    if (!ACTIVE_STATES.has(status))
        return status;
    const lastActivityMs = parseTime(lastActivityAt);
    if (!lastActivityMs || nowMs - lastActivityMs > activeWindowMs)
        return "idle";
    return status;
}
function toRecord(summary, now) {
    const lastActivityAt = summary.lastEventAt || summary.fileModifiedAt || summary.sessionStartedAt;
    return {
...
function toClaudeRecord(summary, now) {
    const lastActivityAt = summary.lastEventAt || summary.fileModifiedAt || summary.sessionStartedAt;
    return {
...
function appServerState(status) {
...
function toPetAppServerRecord(summary) {
...
function preferNewer(left, right) {
    ...
    const evidence = new Map([...left.evidence, ...right.evidence].map((item) => [
        `${item.source}:${item.sessionFile}:${item.processId ?? ""}:${item.processName ?? ""}`,
        item,
    ]));
    return {
        ...preferred,
        evidence: Array.from(evidence.values()).sort((leftEvidence, rightEvidence) => parseTime(rightEvidence.sessionFileModifiedAt) -
            parseTime(leftEvidence.sessionFileModifiedAt)),
    };
}
function dedupeAndSort(records, refreshedAt) {
    ...
    const sessions = Array.from(bySession.values()).sort((left, right) => parseTime(right.lastActivityAt) - parseTime(left.lastActivityAt));
    return { refreshedAt, sessions };
}
function processMatchesRecord(record, observation) {
    const sameStableIdentity = record.processId !== null && record.processId === observation.pid;
    const sameSessionIdentity = Boolean(observation.sessionId) &&
        observation.sessionId === record.sessionId &&
        (observation.provider === null || observation.provider === record.provider);
    if (!sameStableIdentity && !sameSessionIdentity)
        return false;
...
export function enrichAgentSessionRecords(records, observations, options) {
    ...
    return records.map((record) => {
        ...
        if (observation) {
            ...
            const evidence = new Map([...record.evidence, processEvidence].map((item) => [
                `${item.source}:${item.sessionFile}:${item.processId ?? ""}:${item.processName ?? ""}`,
                item,
            ]));
            return {
                ...
                evidence: Array.from(evidence.values()).sort((left, right) => parseTime(right.sessionFileModifiedAt) - parseTime(left.sessionFileModifiedAt)),
            };
        }
...
export async function discoverLocalCodexSessions(options) {
    ...
    return dedupeAndSort(summaries.map((summary) => toRecord(summary, now)), now.toISOString());
}
export async function discoverLocalClaudeSessions(options) {
    const now = (options.now ?? (() => new Date()))();
    const summaries = options.scan({
    ...
    return dedupeAndSort(summaries.map((summary) => toClaudeRecord(summary, now)), now.toISOString());
}
export async function discoverPetAppServerSessions(options) {
    const now = (options.now ?? (() => new Date()))();
    const limit = Math.max(1, Math.min(options.limit ?? 50, 50));
    const summaries = await options.scan(limit);
    return dedupeAndSort(summaries.map(toPetAppServerRecord), now.toISOString());
}
