...
export function safeKnowledgeHandoffError(error) {
...
class PackageValidationError extends Error {
    identity;
    constructor(message, identity = null) {
        super(message);
...
function packageKey(handoffId, packageSha256) {
...
function mediaTypeForPath(relativePath) {
    ...
    if (relativePath === "marker.yaml")
        return "application/yaml";
    if (relativePath.endsWith(".md"))
        return "text/markdown";
    return "text/plain";
}
function parseCompleteFile(content, packageRoot) {
...
function assertIdentity(identity, metadata, packageRoot) {
    ...
    const metadataWorkspaceKey = workspace && typeof workspace === "object" && typeof workspace.workspace_key === "string"
        ? String(workspace.workspace_key)
        : "";
...
function packageManifestHash(records) {
...
async function readJsonFile(filePath) {
    try {
        const value = JSON.parse(await readRegularFile(filePath));
        ...
        throw new PackageValidationError(`无法解析 metadata.json（${safeKnowledgeHandoffError(error)}）`);
    }
}
...
async function assertNoSymlinkInPackagePath(packageRoot, relativePath) {
...
async function inspectPackage(packageRoot) {
    ...
    try {
        ...
        throw new PackageValidationError(`缺少 .complete（${error.code ?? "read_error"}）`);
    }
    ...
    try {
        ...
        throw new PackageValidationError(`无法解析 metadata.json（${safeKnowledgeHandoffError(error)}）`, identity);
    }
    ...
    const records = [];
    const expectedFiles = new Set(["metadata.json", ".complete"]);
    ...
    if (unexpectedFiles.length > 0 || missingFiles.length > 0) {
        throw new PackageValidationError(`交接包文件清单不一致：unexpected=${unexpectedFiles.join(",")}; missing=${missingFiles.join(",")}`, identity);
    }
    const actualPackageSha256 = packageManifestHash(records);
    if (actualPackageSha256 !== identity.packageSha256) {
        throw new PackageValidationError(`package_sha256 不匹配：expected=${identity.packageSha256}; actual=${actualPackageSha256}`, identity);
    }
    ...
    const repositoryRoot = repositoryCapture &&
        typeof repositoryCapture === "object" &&
        typeof repositoryCapture.root === "string"
        ? String(repositoryCapture.root)
        : null;
    return {
...
async function listPackageRoots(handoffRoot) {
    const packageRoots = [];
    ...
    for (const workspaceEntry of workspaceEntries) {
        if (!workspaceEntry.isDirectory() || workspaceEntry.name.startsWith(".tmp-"))
            continue;
        ...
        for (const handoffEntry of handoffEntries) {
            if (!handoffEntry.isDirectory() || handoffEntry.name.startsWith(".tmp-"))
                continue;
...
async function writeQueue(queueFile, items) {
    ...
    try {
        await fsp.writeFile(temporaryPath, `${JSON.stringify({ schema_version: 1, items }, null, 2)}\n`, { encoding: "utf8", flag: "wx" });
...
function packageContentHash(files) {
...
function eventTypeForGitPath(relativePath) {
    ...
    if (normalized.startsWith("refs/") || normalized.includes("/refs/"))
        return "commit";
    return "repository_changed";
}
export function createKnowledgeHandoffTransport(options) {
    ...
    const reconciliationIntervalMs = Math.max(250, options.reconciliationIntervalMs ?? DEFAULT_RECONCILIATION_INTERVAL_MS);
    ...
    async function updateQueueItem(key, updater) {
    ...
    async function scanNow() {
        ...
        for (const packageRoot of packageRoots) {
            try {
                ...
                const validationError = error instanceof PackageValidationError ? error : new PackageValidationError(String(error));
        ...
        if (additions.length > 0) {
            await withQueueLock(async () => {
                ...
                if (newItems.length > 0)
                    await writeQueue(options.queueFile, [...latestQueue, ...newItems]);
            });
        }
        return discovered;
    }
    async function recoverInFlightQueueItems() {
        await withQueueLock(async () => {
            const queue = await readQueue(options.queueFile);
            const timestamp = now().toISOString();
            let changed = false;
            ...
            if (changed)
                await writeQueue(options.queueFile, queue);
        });
    }
    function registerGitWatcher(workspaceKey, repositoryRoot) {
        if (gitWatchers.has(workspaceKey))
            return;
        ...
        try {
            ...
            if (fs.existsSync(commonDirFile) && fs.statSync(commonDirFile).isFile()) {
                ...
                if (commonDir)
                    gitPaths.add(path.resolve(gitPath, commonDir));
            }
        }
        catch {
            return;
    ...
    async function drainOnceImpl() {
        ...
        try {
            ...
            const candidate = queue.find((item) => {
                if (item.status !== "pending" && item.status !== "retryable_failed")
                    return false;
            ...
            try {
                ...
                if (packageInfo.packageSha256 !== inFlight.packageSha256 ||
                ...
                if (!response.ok) {
                    ...
                    if (response.status >= 400 && response.status < 500 && response.status !== 408 && response.status !== 429) {
                        return ((await updateQueueItem(inFlight.key, (item) => ({
                            ...item,
                            status: "rejected",
                            lastError: reason,
    ...
    async function notifyGitEvent(input) {
        try {
    ...
    function stop() {
        ...
        gitWatchers.clear();
        for (const timer of gitHintTimers.values())
            clearTimeout(timer);
        gitHintTimers.clear();
        if (drainTimer)
