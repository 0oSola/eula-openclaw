...
export const NOTIFICATION_PROFILES = ["low", "medium", "high"];
export const MENU_LANGUAGES = ["en", "zh-CN"];
export const PET_AGENTS = ["codex", "claude"];
export const CODEX_LAUNCH_TARGETS = ["vscode-cli", "codex-desktop"];
export const PET_AGENT_LABELS = {
    codex: "Codex",
    claude: "Claude",
};
export const CODEX_ENV_MODES = ["win", "wsl"];
export const CODEX_ENV_MODE_LABELS = {
    win: "Windows",
    wsl: "WSL",
};
...
export function formatSessionMenuLabel(session, now = new Date(), language = "en") {
...
export function normalizeNotificationProfile(value) {
    return NOTIFICATION_PROFILES.includes(value) ? value : "medium";
}
export function normalizeMenuLanguage(value) {
    return MENU_LANGUAGES.includes(value) ? value : "en";
}
function workspaceName(workspacePath) {
    if (workspacePath?.toLowerCase().startsWith("vscode-remote://"))
        return workspaceDisplayLabel(workspacePath);
    const parts = workspacePath?.split(/[\\/]/).filter(Boolean) ?? [];
    return parts.at(-1) || "workspace";
}
function workspaceIdentityKey(workspacePath) {
    return (workspacePath || "").trim().replace(/[\\/]+/g, "\\").toLowerCase();
}
function sessionSeenAt(session) {
    return session.last_seen_at || session.updated_at || null;
}
function sessionRestoreKey(session) {
    return compactText(session.pet_session_id) || compactText(session.codex_session_id);
}
function sessionTaskTitle(session) {
...
function formatActiveWorkspaceMenuLabel(workspace, language) {
    const labels = MENU_LABELS[language];
    ...
    const taskLabel = workspace.activeSessionCount === 1 ? labels.activeTaskSingular : labels.activeTaskPlural;
    return `${workspaceName(workspace.workspacePath)} - ${status} - ${workspace.activeSessionCount} ${taskLabel}`;
}
function formatActiveTaskMenuLabel(task, language) {
    const labels = MENU_LABELS[language];
    const statusKey = String(task.status || "");
    const status = STATUS_LABELS[language][statusKey] || statusKey || labels.unknownStatus;
    return `${task.title} - ${status}`;
}
function buildActiveTaskMenuItem(task, language) {
    return {
...
function buildActiveWorkspaceGroupSubmenu(workspace, isSelected, language) {
    const labels = MENU_LABELS[language];
    const tasks = (workspace.activeTasks ?? []).slice(0, 10);
    const workspaceControl = isSelected
        ? {
...
function buildActiveWorkspaceDirectItems(workspace, isSelected, language) {
    const tasks = (workspace.activeTasks ?? []).slice(0, 10);
    const firstTask = tasks[0];
    const workspaceAction = isSelected
        ? firstTask
...
export function buildActiveWorkspaceSummaries(sessions) {
    ...
    return Array.from(byWorkspace.values())
        .map((workspace) => ({
        ...workspace,
        activeTasks: [...(workspace.activeTasks ?? [])].sort((left, right) => seenAtMs(right.lastSeenAt) - seenAtMs(left.lastSeenAt)),
    }))
        .sort((left, right) => seenAtMs(right.lastSeenAt) - seenAtMs(left.lastSeenAt));
}
export function normalizePetAgent(value) {
    return PET_AGENTS.includes(value) ? value : "codex";
}
export function normalizeCodexEnvMode(value) {
    return CODEX_ENV_MODES.includes(value) ? value : "win";
}
export function normalizeCodexLaunchTarget(value) {
    return CODEX_LAUNCH_TARGETS.includes(value)
        ? value
        : "vscode-cli";
}
export function buildPetMenuModel(options) {
    ...
    const codexDesktopRemoteProjects = (options.codexRemoteWorkspaces ?? []).filter((workspace) => workspace.source === "codex-desktop-ssh");
    const selectedRemoteWorkspace = codexDesktopRemoteProjects.find((workspace) => workspaceIdentityKey(workspace.path) === workspaceIdentityKey(options.selectedWorkspacePath));
    const newSessionLabel = agent === "codex" && codexLaunchTarget === "codex-desktop" && selectedRemoteWorkspace
        ? language === "zh-CN"
    ...
    const currentWorkspaceLabel = language === "zh-CN"
    ...
    const workspaceSubmenu = [
        ...
        ...(options.codexRemoteWorkspaces === undefined
            ? []
            : [
                { id: "workspace:separator:codex-remote", type: "separator" },
                {
                    id: "workspace:codex-remote",
                    label: labels.codexWorkspaces,
                    submenu: codexDesktopRemoteProjects.length > 0
                        ? [
                            {
                            ...
                            ...codexDesktopRemoteProjects.slice(0, 20).map((workspace) => ({
                                ...
                                label: `${workspace.label ?? `${workspace.id} · ${workspaceDisplayLabel(workspace.path)}`}${workspace.availability === "ssh_discovered"
    ...
    return [
        ...
        { id: "send-prompt", label: labels.sendPrompt, action: { type: "send-prompt" } },
        {
            ...
            submenu: recentSessions.length > 0
