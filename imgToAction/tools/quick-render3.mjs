#!/usr/bin/env node
// Render multiple VMDs: load model once, use playVmd but with longer waits
...
let vmds = [], outBase = "imgToAction/outputs/actions/quick3", frames = [10], webUrl = "http://127.0.0.1:3100";
...
async function main() {
  ...
  // Disable physics
  await page.evaluate(() => { window.__mmdCompanionRuntime.hasPhysicsSupport = false; });
  ...
  for (const vmdPath of vmds) {
    ...
    // Use playVmd but wait up to 120s
    ...
    console.log(`  playVmd returned: ${played} in ${((Date.now()-t1)/1000).toFixed(1)}s`);
    ...
    // Wait for isLoadingVmd to clear
    await page.waitForFunction(() => !window.__mmdCompanionRuntime?.isLoadingVmd, null, { timeout: 30_000 });
    console.log(`  VMD ready in ${((Date.now()-t1)/1000).toFixed(1)}s`);
