...
const vmdName = process.env.IMGTOACTION_VMD_NAME || "front_depth_v9e";
const outDirName = process.env.IMGTOACTION_RENDER_DIR || "stages_3_6_v9e_render";
const vmdFiles = [{ name: vmdName }];
const outBase = path.join(projectRoot, "imgToAction", "outputs", "actions", outDirName);
...
async function main() {
  ...
  const allMetrics = []; const allBoneFrames = []; let count = 0;
  for (const vmd of vmdFiles) {
    ...
    for (const frame of testFrames) {
      ...
      if (boneData) {
        const wrist = boneData["右手首"]; const elbow = boneData["右ひじ"]; const lwrist = boneData["左手首"]; const lelbow = boneData["左ひじ"];
        const shoulder = boneData["右肩"] || TPOSE.right_shoulder; const lshoulder = boneData["左肩"] || [0.2471, 6.0384, 0.8999];
        const chin = TPOSE.chin;
        const wo = wrist ? sub(wrist, shoulder) : [0,0,0]; const woL = lwrist ? sub(lwrist, lshoulder) : [0,0,0];
        const wfd = wrist ? dot(wo, FRONT_AXIS) : 0; const wfdL = lwrist ? dot(woL, FRONT_AXIS) : 0;
        ...
        const m = { vmd: vmd.name, frame, wrist_front_depth: Math.round(wfd*1000)/1000, left_wrist_fd: Math.round(wfdL*1000)/1000, wrist_to_chin: Math.round(wtc*1000)/1000, elbow_angle: Math.round(ea*10)/10 };
        allMetrics.push(m);
        // Export full bone world coordinates for gate validation
        allBoneFrames.push({ index: frame, joints: boneData });
        console.log(`OK ${vmd.name} f${frame}: wfd=${m.wrist_front_depth}, lfd=${m.left_wrist_fd}, chin=${m.wrist_to_chin}, ea=${m.elbow_angle}`);
      }
