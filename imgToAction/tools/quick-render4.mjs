#!/usr/bin/env node
// Render multiple VMDs: load model once, set clip directly without playVmd
...
let vmds = [], outBase = "imgToAction/outputs/actions/quick4", frames = [10], webUrl = "http://127.0.0.1:3100";
...
async function main() {
  ...
  for (const vmdPath of vmds) {
    ...
    // Fetch VMD as ArrayBuffer, then use MMDLoader's parser to build animation clip
    const loaded = await page.evaluate(async (url) => {
      ...
      // Fetch VMD file as arraybuffer
      const response = await fetch(url);
      const buffer = await response.arrayBuffer();

      // Use loader's internal parser to parse VMD
      ...
      // Set clip directly, skip playVmd/helper.add
      rt.currentClip = clip;
      rt.setCalibrationCaptureMode?.(true);
      return { ok: true, tracks: clip?.tracks?.length || 0 };
    }, vmdUrl);

    console.log(`  Loaded: ${JSON.stringify(loaded)} in ${((Date.now()-t1)/1000).toFixed(1)}s`);
    if (loaded?.error) {
      console.log(`  Error: ${loaded.error}, skipping`);
      continue;
    }
