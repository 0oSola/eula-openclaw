#!/usr/bin/env node
// Minimal batch renderer: load model once, render multiple VMDs
...
function startStaticServer(root) {
  const server = createServer((req, res) => {
    ...
    res.writeHead(200, {"Access-Control-Allow-Origin":"*","Content-Type": types[ext]||"application/octet-stream"});
...
let vmds = [], outBase = "imgToAction/outputs/actions/quick", frames = [10], webUrl = "http://127.0.0.1:3100";
...
async function main() {
  ...
  for (const vmdPath of vmds) {
    ...
    await page.evaluate(async (url) => {
      ...
      rt.hasPhysicsSupport = false; await rt?.playVmd?.(url, 1, [], { disableCrossfade: true });
      rt?.setCalibrationCaptureMode?.(true);
    }, vmdUrl);
    ...
    for (const frame of frames) {
      ...
      console.log(`  frame ${frame}: ${JSON.stringify(metrics["右足首"]||"N/A")}`);
    }
