...
export type CompletionNoticeRuntime =
  | "app-server"
  | "codex-desktop"
  | "cli"
  | "wsl"
...
export type CompletionNoticeActionResult =
  | {
      ok: true;
      mode: "restore-requested" | "follow-up-sent" | "stop-requested" | "copy-only";
      turnId?: string;
    }
  | {
      ok: false;
...
export type CompletionNoticeWindowState = {
  notices: CompletionNotice[];
  expanded: boolean;
};

export type CompletionNoticeReducerState = CompletionNoticeWindowState & {
  dismissedKeys: string[];
};

export type CompletionNoticeAction =
  | { type: "status"; status: CompletionNoticeStatus; agentLabel?: string }
  | { type: "expand" }
  | { type: "collapse" }
...
function normalizeDismissedKeys(values: readonly unknown[]): string[] {
  const keys: string[] = [];
  ...
  for (
    ...
    const key = typeof values[index] === "string" ? compact(values[index] as string) : "";
...
export function parseDismissedCompletionNoticeKeys(value: string | null | undefined): string[] {
  ...
  try {
    ...
    if (Array.isArray(parsed)) return normalizeDismissedKeys(parsed);
    if (typeof parsed === "string") return normalizeDismissedKeys([parsed]);
    if (parsed && typeof parsed === "object" && "dismissedKeys" in parsed) {
      const dismissedKeys = (parsed as { dismissedKeys?: unknown }).dismissedKeys;
      if (Array.isArray(dismissedKeys)) return normalizeDismissedKeys(dismissedKeys);
    }
    return [];
  } catch {
...
export function addDismissedCompletionNoticeKey(keys: readonly string[], key: string): string[] {
...
export function buildCompletionNotice(
  ...
  if (status.state !== "completed" || !key || !workspacePath) return null;
...
export function completionNoticeSummary(notice: CompletionNotice): string {
  return notice.outputLines[0] || "Completed";
}

export function createCompletionNoticeReducerState(
  dismissedKeys: readonly string[] = [],
): CompletionNoticeReducerState {
  return {
    notices: [],
    expanded: false,
...
export function reduceCompletionNoticeState(
  state: CompletionNoticeReducerState,
  action: CompletionNoticeAction,
): CompletionNoticeReducerState {
...
export function completionNoticeWindowSize(
  noticeCountOrExpanded: number | boolean,
  expandedOverride?: boolean,
): { width: number; height: number } {
  const noticeCount = Math.min(
    MAX_VISIBLE_COMPLETION_NOTICES,
    Math.max(1, typeof noticeCountOrExpanded === "number" ? Math.floor(noticeCountOrExpanded) : 1),
  );
  const expanded =
    typeof noticeCountOrExpanded === "boolean"
...
export function createCompletionNoticeBrowserWindowOptions(
  preloadPath: string,
...
export function calculateCompletionNoticePosition(
  petBounds: Rectangle,
  noticeSize: Pick<Rectangle, "width" | "height">,
  workArea: Rectangle,
  gap = COMPLETION_NOTICE_WINDOW_GAP,
): { x: number; y: number } {
...
export function toCompletionNoticeWindowState(
  state: CompletionNoticeReducerState,
): CompletionNoticeWindowState {
  return {
