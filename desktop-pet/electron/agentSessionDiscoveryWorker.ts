...
export type AgentSessionDiscoveryWorkerRequest = {
  codexHome: string;
  wslCodexHome: string;
  claudeHome: string;
  apiBaseUrl: string;
...
export type AgentSessionDiscoveryWorkerCandidate = {
  session: AgentSessionRecord;
  agentHome: string | null;
};

export type AgentSessionDiscoveryWorkerResult = {
  candidates: AgentSessionDiscoveryWorkerCandidate[];
  processObservations: AgentProcessObservation[];
...
async function discover(
  request: AgentSessionDiscoveryWorkerRequest,
): Promise<AgentSessionDiscoveryWorkerResult> {
  ...
  const candidates: AgentSessionDiscoveryWorkerCandidate[] = [];
...
parentPort.once("message", async (request: AgentSessionDiscoveryWorkerRequest) => {
  try {
