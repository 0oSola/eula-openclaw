...
export type CodexWorkItemKind = "goal" | "agent_update" | "file_change" | "approval" | "error";
export type CodexMethodKind = "tool" | "command" | "check" | "file_operation" | "approval";
export type CodexMethodOutcome = "pending" | "success" | "failed" | "unknown";
...
export type CodexReviewFacts = {
  ...
  function_call_summaries: Array<{
    name: string;
    command: string | null;
  }>;
  work_items: Array<{
    kind: CodexWorkItemKind;
    source: "user" | "assistant" | "tool" | "event";
    text: string;
    timestamp: string | null;
  }>;
  methods: Array<{
    kind: CodexMethodKind;
    name: string;
    command: string | null;
    outcome: CodexMethodOutcome;
...
type ParseOptions = {
  chunkBytes?: number;
  maxLineChars?: number;
  /** @deprecated Full streaming scan no longer uses head/tail windows. */
  maxHeadBytes?: number;
  /** @deprecated Full streaming scan no longer uses head/tail windows. */
  maxTailBytes?: number;
};
...
function boundedCompactFactText(value: unknown, maxLength: number): string | null {
  const text = redactSensitiveText(compactText(value));
  if (!text) return null;
  return Array.from(text).slice(0, maxLength).join("");
}

function eventTimestamp(event: CodexSessionJsonEvent): string | null {
...
function forEachJsonLineEvent(
  filePath: string,
  options: ParseOptions,
  visit: (event: CodexSessionJsonEvent) => void,
) {
...
function resolveSessionStartedAt(earliestTimestamp: string | null, stat: fs.Stats): string {
...
function isToolCallPayloadType(value: unknown): boolean {
  return value === "function_call" || value === "custom_tool_call";
}

function isToolCallOutputPayloadType(value: unknown): boolean {
...
function decodeQuotedJavaScriptString(value: string, quote: string): string {
  if (quote === '"') {
    try {
...
function commandFromCustomToolInput(value: unknown): string | null {
  ...
  if (cmdMatch) return compactText(decodeQuotedJavaScriptString(cmdMatch[2], cmdMatch[1])) || null;
  const commandMatch = trimmed.match(/\bcommand\s*:\s*(["'`])((?:\\.|(?!\1)[\s\S])*?)\1/);
  if (commandMatch) return compactText(decodeQuotedJavaScriptString(commandMatch[2], commandMatch[1])) || null;
  return compactText(trimmed) || null;
}

function parseToolCallCommand(payload: Record<string, unknown>): string | null {
  ...
  if (payload.type === "custom_tool_call") return null;
...
function inferToolOutcome(
  callPayload: Record<string, unknown> | null,
  outputPayload: Record<string, unknown>,
  output: string,
): { outcome: CodexMethodOutcome; exitCode: number | null } {
...
function patchChangedFiles(payload: Record<string, unknown>): string[] {
  const callName = compactText(payload.name);
  if (!/patch/i.test(callName)) return [];
...
function addWorkItem(
  facts: CodexReviewFacts,
  event: CodexSessionJsonEvent,
  kind: CodexWorkItemKind,
  source: "user" | "assistant" | "tool" | "event",
  value: unknown,
) {
  if (facts.work_items.length >= REVIEW_FACT_MAX_WORK_ITEMS) return;
  const text = boundedCompactFactText(value, REVIEW_FACT_MAX_WORK_ITEM_CHARS);
  if (!text) return;
  if (facts.work_items.some((item) => item.kind === kind && item.text === text)) return;
...
function changedFileValues(value: unknown): unknown[] {
  if (Array.isArray(value)) {
    return value;
  }
  return [value];
...
function addChangedFileFacts(
  facts: CodexReviewFacts,
  event: CodexSessionJsonEvent,
  value: unknown,
  source: "tool" | "event" = "event",
) {
...
function isCheckCommand(command: string | null): boolean {
...
function methodKindForCall(name: string, command: string | null): CodexMethodKind {
  if (isCheckCommand(command)) return "check";
  if (command) return "command";
  if (/edit|write|patch|file/i.test(name)) return "file_operation";
  return "tool";
}
...
function addMethod(
  ...
  if (facts.methods.length >= REVIEW_FACT_MAX_METHODS) return null;
  facts.methods.push({
    kind: method.kind,
...
type ReviewFactsAccumulator = {
  facts: CodexReviewFacts;
  pendingCalls: Map<string, PendingToolCall>;
  lastCall: PendingToolCall | null;
};

function createReviewFactsAccumulator(): ReviewFactsAccumulator {
  return {
    facts: {
      failed_commands: [],
      changed_files: [],
...
function rememberPendingCall(accumulator: ReviewFactsAccumulator, call: PendingToolCall) {
...
function processReviewFactEvent(accumulator: ReviewFactsAccumulator, event: CodexSessionJsonEvent) {
  ...
  if (
    ...
    if (assistantText && !facts.assistant_messages.includes(assistantText)) {
      facts.assistant_messages.push(
        truncateText(assistantText, REVIEW_FACT_MAX_ASSISTANT_MESSAGE_CHARS) || assistantText,
      );
  ...
  if (event.type === "response_item" && isToolCallOutputPayloadType(payload.type)) {
    ...
    if (callId) accumulator.pendingCalls.delete(callId);
  }
...
export function extractCodexReviewFacts(events: CodexSessionJsonEvent[]): CodexReviewFacts {
  ...
  for (const event of events) processReviewFactEvent(accumulator, event);
  return accumulator.facts;
}

function nextCodexSessionStatus(
  current: CodexSessionStatus,
  event: CodexSessionJsonEvent,
): CodexSessionStatus {
  const payload = asRecord(event.payload);
  ...
  if (isApprovalEvent(event)) return "waiting_approval";

  if (event.type === "response_item") {
    if (isToolCallPayloadType(payloadType)) return "command_running";
    if (/file.*change|patch|diff/i.test(payloadType)) return "file_changed";
    if (isToolCallOutputPayloadType(payloadType) || payloadType === "reasoning") return "running";
    if (payloadType === "message" && role === "assistant") return "completed";
    if (payloadType === "message" && role === "user") return "running";
    return current;
  }

  if (event.type !== "event_msg") return current;
  if (payloadType === "task_complete" || payloadType === "agent_message") return "completed";
  if (payloadType === "task_started" || payloadType === "user_message") return "running";
  if (/file.*change|patch|diff/i.test(payloadType)) return "file_changed";
  if (/process.*exit|session.*closed|disconnected/i.test(payloadType)) return "disconnected";
  if (/failed|error/i.test(payloadType)) return "failed";
  return current;
}
...
export function inferCodexSessionStatus(events: CodexSessionJsonEvent[]): CodexSessionStatus {
  ...
  for (const event of events) status = nextCodexSessionStatus(status, event);
  return status;
}
...
function extractSessionMeta(event: CodexSessionJsonEvent): CodexSessionMeta | null {
...
function absolutePathWithComparableDrive(value: string): string {
...
function workspaceRelativeChangedFile(value: string, workspacePath: string): string | null {
  const trimmed = value.trim();
  if (!trimmed) return null;
  ...
  if (looksAbsolute) {
    ...
    if (!workspaceAbsolute || !fileAbsolute.toLowerCase().startsWith(workspacePrefix.toLowerCase())) return null;
...
function normalizeReviewFactPaths(facts: CodexReviewFacts, workspacePath: string): CodexReviewFacts {
  ...
  for (const file of facts.changed_files) {
    ...
    if (!normalizedFiles.includes(normalized)) normalizedFiles.push(normalized);
  }
  facts.changed_files = normalizedFiles;
  facts.work_items = facts.work_items.filter((item) => {
    if (item.kind !== "file_change") return true;
...
export function parseCodexSessionFile(filePath: string, options: ParseOptions = {}): CodexSessionFileSummary {
  ...
  const sessionState: { meta: CodexSessionMeta | null } = { meta: null };
  let firstPromptPreview: string | null = null;
  let lastSummary: string | null = null;
  let lastOutput: string | null = null;
  let lastStatus: CodexSessionStatus = "running";
...
export function scanRecentCodexSessionFiles(options: {
  ...
  for (const candidate of candidates) {
    ...
    if (seenFilePaths.size >= maxFiles) break;
