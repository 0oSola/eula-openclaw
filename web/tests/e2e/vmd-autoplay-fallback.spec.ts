...
test("a model without its own favorite idle plays the read-only shared idle instead of returning to bind pose", async ({ page }) => {
  ...
  await expect
    .poll(() => page.locator(".mio-stage").getAttribute("data-active-vmd-url"))
    .toContain("/assets/vmd/file/eula-idle");

  const panel = page.getByTestId("mio-advanced-panel");
  await expect(panel.getByText("eula-idle.vmd", { exact: true })).toHaveCount(0);
});
