# Stage 2C-M2a 权威取证（只读）：打开权威 V14D .blend，自省 Brows/Lashes 材质节点，
# 记录 BaseColor 纹理、色彩空间、alpha/混合/裁剪口径、BaseColor/节点公式、双面与深度排序需求。
#
# 用法：
#   blender --background --python web/scripts/forensic-v14d-brows-lashes.py -- \
...
def describe_material(mat):
    ...
    out = {
        ...
        "useBackfaceCulling": getattr(mat, "use_backface_culling", None),
...
def main():
    ...
    for name in ("PROTO_GF2_Brows", "PROTO_GF2_Lashes"):
    ...
    print("FORENSIC-BROWS-LASHES-OK -> " + str(out_path))
