...
Electron 原生右键菜单当前由主进程构建，API 地址来自 `MMD_PET_API_BASE_URL`（默认 `http://127.0.0.1:8000`），菜单查询身份来自 `MMD_PET_USER_ID`（默认 `admin-1`）。菜单每次打开都会请求 `GET /desktop-pet/sessions?limit=10` 刷新最近 session，并把显示标签压缩成 title / time / status 组合，避免在菜单里暴露 UUID-heavy 字符串。当前菜单项包括 `New Codex Session`、`Recent Sessions`、`More Sessions...`、`Interaction Mode`、`Notification Detail`、`Focus VSCode Terminal`、`Retry API` / `Retry API Connection` 和 `Close`。其中 `Interaction Mode` 的 `Drag Whole App` / `Adjust Camera` 会立即生效：默认把 shell、stage 和 WebGL canvas 标记为 native `-webkit-app-region: drag`，保证透明 frameless window 可拖动；主进程拦截 Windows/Linux `system-context-menu` 并用窗口内坐标弹出 pet 菜单，避免显示系统菜单。切到相机调整时 renderer 把 stage/canvas 改为 `no-drag`，让 MMD stage 的相机控制继续工作。`Notification Detail` 当前只保存在 Electron 主进程内存并通过 renderer 状态提示反映；`Retry API` 会让 renderer 重新读取模型/动作/shared-config。`New Codex Session`、`Recent Sessions` restore、`More Sessions...` 和 `Focus VSCode Terminal` 目前只向 renderer 发出显式 not-wired status，占位等待后续 VSCode 启动桥、Codex CLI 启动和 resume 工作流接入。完整 desktop-pet 拓扑留到 Task 22 补充。
