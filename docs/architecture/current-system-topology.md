...
本修正轮已用四份证据缺失/错误 fps、NaN、Infinity、null 及 provenance 缺失/错误负测锁定该硬约束；正式报告中的 pixel/triUV 原子 evidence 是 Gate 的消费输入，不是仅供展示的派生字段。
