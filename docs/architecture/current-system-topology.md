...
Stage 2B-P1（`codex/v14d-reze-fresh-patch-completeness`，2026-09-01）完成 `patch-reze-engine.mjs` 的 fresh-patch 注入完整性闭环：`--self-test` 在真实隔离的 `reze-engine@0.26.0` fixture 上要求首次 exit=0、二次 exit=0、二次文件哈希不变，并对 anchor-miss、missing-file、重复 marker、断点 A 缺失、断点 B 缺失全部硬拒绝；普通补丁、predev、prebuild 与 `--verify` 共用严格 marker 计数，56 项 Stage 2B 不变量必须各恰好一次。生产注入现在覆盖 materialAuxTextures 类型、断点 A 的 override/import/接线、断点 B 的 binding(5) baseEntries/fallback/重绑展开、bind-group layout 及 src/dist 路径；真实 `web/node_modules` 仅由脚本按需重打，fresh fixture 首次/二次和 `npm run build` 均通过。详见 `docs/handoff/2026-09-01-v14d-reze-fresh-patch-completeness.md`。
