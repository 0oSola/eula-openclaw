...
Stage 2B-M1（`codex/v14d-face-state2-runtime`，2026-08-31）详见 1.1 节与本票交付报告 `docs/handoff/2026-08-31-v14d-face-state2-runtime.md`：实时合成链已接线并通过配准 Gate（fov=28.0725 精确匹配、cameraPos/Target 非空、负测 shift/null 判别失败）与双纹理绑定 Gate（liveBound=true、liveMaskPath=Textures/v14d-state2-mask/state2.png），full Face 覆盖率 1.0、pageErrors=0；但完整 Face Gate MAE 未达 ≤20/255（faceShadowOnly [58.41,54.83,62.96]、finalFaceComposite [50.96,26.51,29.41]），阈值未放宽、按阻塞交付，根因分析与修正路线见交付报告。
