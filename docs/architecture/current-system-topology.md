...
Stage 2C-M1（`codex/v14d-hairab-stage`，2026-09-03）是当前唯一权威的生产变体定义：在权威克莱妲 PMX（`GirlsFrontline KoledaDefault.pmx`）+ State2 mask 资格满足时，V1 从原分组抽出 Face、BodySkin、HairA、HairB 并分别绑定 V14D graph；默认仍为 original，按「用户 + 模型 + reze-k3 管线」隔离持久化，非克莱妲、缺 mask 或 applyStyleGroups 失败时安全回退 original。Face/BodySkin 继续使用既有实时合成，HairA/HairB 使用 `V14D_HAIR_V1_COMPOSITE_GRAPH`（renderClass=hair）与独立 `V14D_HAIR_HELPER_WGSL`，canvas dataset 暴露四槽 draw-call 绑定计数。当前迁移进度为 **4/15（Face、BodySkin、HairA、HairB）**，其余 11 槽仍未迁移；Cth* 衣物/装备、Glock、GunSilencer 永久排除。
Stage 2C-M1 的视觉目标是权威 BaseColor 公式 `targetLinear(uv)=srgbToLinear(hair_d(uv))×[0.84,0.85,0.96]`；Blender 取证 JSON 机器输出 HairA/HairB 的 Anisotropic=0.7200000286、Roughness/Specular MapRange 参数、ToonRamp 三段参数、ShaderToRGB/Tangent/Alpha 链，本阶段只迁移 BaseColor 乘色，未宣称复现视角相关高光。真实 `/companion` G1-G6 全过：HairA materialId=24、samples=4,189、coverage=0.037214、origMae=40.879→v1Mae=32.181、drop=0.2128；HairB materialId=25、samples=15,426、slotPixels=15,495、coverage=0.096902、origMae=53.705→v1Mae=39.132、drop=0.2714。目标阈值为 samples≥30、drop>0.05、v1Mae<90，变化阈值为 mae>1 且 maxMeanDiff>1；样本来自 engine-pick-material-id-depth 对应材质 ID + 深度前景 pass，错槽交换负测严格互换。wrongTint `[1.35,0.25,0.25]` 的 HairA/HairB 正式误差分别为 53.535/51.806（drop=-0.3096/0.0354），必须通过 `negativeVerdict.status=rejected`、两槽正式目标判据均 false 且无其他 failures；场景快照逐字段覆盖 settings、grade、backgroundEffect、transparentBackground、viewTransform（exposure/gamma/look）。证据位于 `web/.scratch/reze-k3-v1-stage/` 与交付报告。
