...
`reze-k3` 舞台提供可选皮肤变体「原始 Reze K3 / Reze K3 V1 皮肤预览（V14D）」：仅当本地目录导入（localModelImport）的权威克莱妲 PMX（`GirlsFrontline KoledaDefault.pmx`）且目录内含权威 State2 mask（`v14d-01234-face-shadow-state-2.png`）时，渲染模式选择器下方出现切换；选择 V1 触发 `RezeWebGpuStage` boot 重建，把 Face/BodySkin 从现有分组抽出并绑定到 V14D 实时合成 graph（Face State2 finalComposite + BodySkin warm composite，与诊断入口同源、共享引擎补丁五的 WGSL 覆写），其余材质保持 reze-k3 正常分组。V1 默认关闭（original），按「用户 + 模型 + reze-k3 管线」隔离持久化在 localStorage（`mmd_reze_k3_skin_variant_v1:<userId>:reze-k3:<modelPath>`），不写入会话或桌面 Pet 共享配置；非克莱妲模型或 mask 缺失时安全回退 original，不抛错、不泄漏到 reze-design 或其他管线。运行时经 canvas dataset 暴露 `v14dSkinVariant` 与 Face/BodySkin 的 draw-call 级绑定证据（`v14dSkinVariant{Face,Body}{DrawCalls,OnComposite}`）。**范围纠偏（2026-09-02，历史：已被下方 Stage 2C-M1 取代）**：截至该日期该变体只迁移 Face/BodySkin 两个 PMX 材质槽（皮肤阶段，2/15）；最终目标是「Reze K3 + V14D 角色外观材质（15 槽）」——Face、BodySkin、Brows、Lashes、Emotions、HairA、HairB、Eyes、EyeWhite、EyeShadow、Eyes+、UpperTeeth、LowerTeeth、Tongue、FingerNails（其余 13 槽截至 2026-09-02 未迁移）；所有 Cth* 衣物/披风/手套/裤/鞋/口罩/包与 Glock、GunSilencer 武器装备明确排除、永久保持原始 Reze K3；舞台环境（K3 灯光、星空背景、自由相机、动态 VMD）始终保留，与角色材质迁移范围相互独立。概念登记见 `workflow/concepts/reze-k3-skin-variant.zh-CN.md`。 真实舞台验收（2026-09-02，`codex/reze-k3-v1-stage-visual-acceptance`）在 `/companion` 存根环境端到端（Playwright route 存根 API）通过 G1-G6 六项运行时 Gate（用户路径/真实绑定/视觉 A-B/持久化/VMD 零回归/管线隔离）+ 独立工程门禁（当时统称 G1-G7），证据见 `docs/handoff/2026-09-02-reze-k3-v1-stage-visual-acceptance.md`。验收期间落地两项最小修复：(a) `patch-reze-engine.mjs` 移除 reze-engine `pmx-loader` 的 1000 字节文本硬上限（权威克莱妲 PMX 含合法超长注释字符串，offset 647 触发 `Suspicious string length: 1006` 阻断加载）；(b) `RezeWebGpuStage` 新增只读验收探针——canvas dataset `vmdPlayback{Name,Current,Duration,Playing}` 与 `window.__rezeStageProbe`（playVmd/pauseVmd/seekVmd），用于 reze-k3 无 `__mmdCompanionRuntime` 时断言 VMD load→play→pause→seek 的真实帧前进，探针仅在显式验收开关 ?v14dAcceptanceProbe=1 下挂载（生产默认不暴露），rAF 进度镜像带取消标志、卸载即清理；第二轮另加负测钩子 applyBadSkinGraph("wrongGraph"|"failCompile")（同样仅验收开关），真实驱动「错误 graph 不命中 Face/BodySkin draw-call」「applyStyleGroups 失败回退 original」负测。第四轮收口 VMD 完成回调运行时契约：handleRezeVmdFinished 的身份/模型有效性校验（currentName===finishedName 且 model===modelRef.current）现在先于任何 clear/count/loop/reset/complete 副作用——过期/错误 finishedName 的回调不再清除当前动作已 arm 的完成兜底、不自增完成计数、不改 currentVmdUrl；pauseVmd/seekVmd 会取消完成兜底计时器，避免中段暂停/跳走被兜底误判为自然播完。共享函数 loadVmdThroughInteractionPath 把请求守卫作为 guard 回调在 await loadVmd 之后、任何 apply/play/currentUrl/fallback/resetPhysics 副作用之前硬检查，较慢完成的旧请求无副作用退出。新增仅 v14dAcceptanceProbe=1 可见的探针契约 fireStaleFinish(staleName)（注入过期完成回调）与 vmdFallbackState()（返回兜底 armed/fired/delay/currentName/timerActive 证据），生产默认不暴露；对应负测证明「注入真实 A 名（race-a.vmd）的过期回调不清当前 fallback、完成计数不增、vmdNaturalFinishName 不改 A、B 后续正常完成」与「竞态 A慢/B快 下旧请求 A 无副作用退出（finishDelta===0 硬断言，--self-test-g5-race 负向自验判别力）」。已核实正常 VMD 加载路径在 loadVmd 完成后对所有模式调用 engine.resetPhysics()（无需修复）。
Stage 2C-M1（`codex/v14d-hairab-stage`，2026-09-03）把 HairA/HairB 两个 PMX 材质槽迁入 V1 变体：引擎新增独立 `V14D_HAIR_HELPER_WGSL` 常量（`v14d_hair_composite(base)=base×[0.84,0.85,0.96]`，常量来自权威 blend 取证 `web/scripts/forensic-v14d-hair-state.py`），assembleModule 新增 `includeV14dHairHelper` 参数单独注入（不连带 State2 mask 声明/binding 5，hair graph 无需 mask），compile 按 graph.name "V14D Hair V1 Composite" 覆写 final_color；生产 V1 把 HairA/HairB 抽出绑定到 `V14D_HAIR_V1_COMPOSITE_GRAPH`（renderClass=hair），dataset 新增 `v14dSkinVariant{HairA,HairB}{DrawCalls,OnComposite}` 分区绑定证据。头发视角相关部分（Anisotropic 0.72、Roughness/Specular MapRange、ToonRamp 经 ShaderToRGB）按 A/B/C 分类为 C（不能烘焙/不固化视角高光），本阶段只迁移 BaseColor 乘色。迁移进度更新为 **4/15（Face、BodySkin、HairA、HairB）**，其余 11 槽仍未迁移。验收（真实 `/companion` + 权威 PMX/VMD/State2 mask）G1-G6 全过：HairA/HairB draw-call 各 1/1 命中目标 graph、missing/wrongGraph/wrongMaterial/failCompile 负测均检出、头发区显著变化（mae=10.5/maxMeanDiff=11.1）、衣服/装备/星空经同变体连拍噪声基线校准后稳定（clothes ROI 收窄排除刘海遮挡区帧间微动）、VMD original/V1 双模式 load→play→pause→seek→自然结束零回归、场景字段 original/V1 逐字段一致。证据 `web/.scratch/reze-k3-v1-stage/gate-report.json` 与交付报告 `docs/handoff/2026-09-03-reze-k3-v1-hairab-stage.md`。
