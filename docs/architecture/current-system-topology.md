...
Stage 2A-M1.1（`codex/v14d-face-uv-texel-align`）在同一入口追加一个只读诊断能力：`v14dFaceMode=uvDebug` 把 Face 材质切到几何 UV 调试图（fragment 输出插值 `input.uv` 的 R=u、G=v），`window.__v14dFaceStatic.exportFaceUvPng()` 用与 `exportFaceMaskPng()` 相同的 Face pick mask 导出逐像素 UV PNG（仅 faceStatic 模式、默认关闭、不改变三模式语义与生产渲染）。配套 UV-direct 对账脚本（`.scratch/v14d-face-static/uv-align/`）从权威 `.blend` 导出 Face 材质 frame-120 逐 loop UV/屏幕投影与线性像素快照，建立同 UV 逐纹素对账；当前对账定位的剩余差异主导项是 Web 固定相机（`V14D_FACE_STATIC_CAMERA`）与 Blender `PROTO_GameCamera` 的几何/姿态口径差异（脸部屏幕覆盖宽 5.2×/高 8.6×、质心偏移约 298px），而非纹理采样、过滤、mipmap 或颜色管理（已逐项单变量排除）。后续闭合 Blender/Web 脸部颜色口径需先对齐相机/姿态几何。
