...
Stage 2C-M1.1（codex/v14d-hairab-triuv-gate，2026-09-03）是当前 HairA/HairB 正式门禁定义：每个正式屏幕样本必须同时命中 engine-pick-material-id-depth 的对应材质 ID、同槽非索引展开 triId、插值 UV 及其三角形 UV 重心校验；目标由权威 hair_d 同 UV 线性双线性采样 × v14dAuthority.js 唯一导出的 V14D_HAIR_TINT，再转显示字节得到。旧 targetMean 仅保留为历史 checkpoint，正式 Gate 不再读取它。真实 /companion G1-G6 全过：HairA materialId=24、samples=4,185、coverage=0.037179、origMae=28.366→v1Mae=25.248、drop=0.1099、p95=71；HairB materialId=25、samples=15,502、coverage=0.096946、origMae=47.589→v1Mae=35.744、drop=0.2489、p95=99.333→89。两槽正式 target binding 均 consistent=true。错槽/错目标交换负测真实 analyzer exit=1、negativeVerdict.status=rejected、HairA/HairB 正式 Gate 均 false、analysisFailures=[]；wrongTint 保留 analyzer exit=0、同样的预期拒绝状态且 failures=[]。captureHairTriUv 与所有诊断 pass 只在显式 acceptance probe 下启用，读取后恢复渲染循环/原有动画状态；本阶段只迁移 BaseColor，不迁移视角相关高光/各向异性/ToonRamp/ShaderToRGB，也不修改 PMX、VMD、骨骼、权重、Morph、IK、Grant、Physics、拓扑或材质槽。证据位于 web/.scratch/reze-k3-v1-stage/、visual-diff.json、visual-diff-swap-slot-target.json、visual-diff-wrongtint.json 与交付报告。
