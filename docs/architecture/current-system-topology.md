...
**验收修正轮（2026-09-02，同 failure family 二次闭合）**：来源主会话验收要求机制 Gate 真实闭合。修正并复验通过（正式 Gate exit 0，===BODY-SKIN-GATE-OK===）：(1) 版本统一为 v1（V14D_BODY_SKIN_BONE_REGIONS_V1 与内部 version 字段一致，同步概念/术语表/本文档）；(2) 语义负测改为「在真实 PMX joints/weights 上用扰动骨骼集合重跑同一归属函数（classifyV14dVerticesByBoneRegion 的可选 boneSetsOverride）并断言 Gate 判定翻转」，N1 左右手交换（解剖对称各 882 三角形，断言语义对调=swap.leftMae≈base.rightMae 且异于 base.leftMae）、N2 腰腹骨骼 6 注入左手（leftHand 882→1098、torso 216→0、逐像素 MAE 变化）、N5 错骨序（剔除 torso 骨骼 6 后 torso 归属塌缩为 0）全部检出；像素归属复用主采集全身 triId/UV 空间、只替换 boneRegionLabels，保证扰动为唯一变量（修正 rerun 用近景相机重渲导致 triId 与 main HDR 投影错位的缺陷）；(3) N3 删除 `|| true` 软通过，face_d 缺失改启动时 GATE-CONFIG-FAIL（exit 2）；(4) neck/torso 与左右手一样输出非 null 的 regionTriTotal、coverageNumerator/coverageDenominator、coverage（coverage=命中像素数/该语义区域三角形总数，量纲为像素/三角形、非 0..1 面积占比，报告含 coverageNote），正式报告无 coverage=null；(5) 骨骼索引→骨名硬断言（V14D_BODY_SKIN_BONE_NAME_ASSERT，34 索引正则匹配 skeletonBoneNames，0 失配），防骨序漂移静默错分；(7) 双线性保留但措辞纠正——四区域当前 MAE（neck/torso/leftHand/rightHand 均 <0.07）下不再声称「颜色/光照是唯一剩余根因」，报告 remainingRootCause 标为 color-gate-unresolved、列 mip/LOD/sampler/色彩空间链为未排除项；(8) patch-reze-bone-names.mjs 更名 probe-reze-bone-names.mjs（只读探针）并参数化 reze-engine 路径（默认 web/node_modules，可 REZE_ENGINE_DIR 覆盖），不再硬编码 D:/workspace；(9) Git 跟踪范围清理（移除 .scratch 冗余 baseline/v2 轮与一次性 append helper，保留可重复正式脚本、机器 JSON、概念/架构/交付文档与 gate-m31-final 截图/报告）；(10) 正式 Gate 持续消费真实 draw-call 证据（exportBodySkinDrawBinding.allBodySkinOnComposite），missing/wrongGraph/wrongMaterial 运行时负测全部非零退出（gate-v14d-body-graph-runtime-negative ===RUNTIME-NEG-OK===）。复验：npm run build exit 0（62 不变量）、正式 BodySkin Gate exit 0、N1/N2/N3/N5 + graph 运行时负测全过、probe-v14d-face-default exit 0（默认生产入口 faceStatic 旁路）、probe-v14d-vmd-runtime exit 0（load→play→pause→seek 正常）、git diff --check exit 0、PMX/VMD/动画运行时零 diff。
