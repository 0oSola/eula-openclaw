...
本修正轮的实现/测试提交为 `2dd2404b90f0fb580f20f9dbb00009f74d51d584`。正式 `/companion` G1-G6 全过：HairA materialId=24、samples=4,185、coverage=0.037179、origMae=28.394→v1Mae=25.250、drop=0.1107、p95=71.267→71；HairB materialId=25、samples=15,503、coverage=0.096952、origMae=47.743→v1Mae=35.746、drop=0.2513、p95=99.333→89。两槽正式 `targetBinding.consistent=true` 且 `inputsValid=true`，正常模式的同像素 canonical target penalty 为 0。错槽/错目标交换负测使用另一槽完整、真实且 triUV 合法的目标流，不再写入强制失败布尔值：analyzer exit=1、`negativeVerdict.status=rejected`、两槽正式/自然指标 Gate 均 false、`inputsValid=true`、`analysisFailures=[]`；HairA 的错目标 `drop=0.0469`、`v1Mae=40.351` 对 canonical `25.250` 的 penalty=15.100、P95=79>71，HairB 的 penalty=4.185、P95=89.667>89。wrongTint 按既有协议 analyzer exit=0、`negativeVerdict.status=rejected`、两槽正式 Gate=false、`analysisFailures=[]`，错误画布 HairA `v1Mae=63.950`/drop=-1.2522、HairB `v1Mae=56.668`/drop=-0.1869。
`captureHairTriUv` 与所有诊断 pass 只在显式 acceptance probe 下启用；采集前快照 VMD `currentSeconds`、`playing`/`paused` 和 render-loop 运行态，采集成功、提前返回或异常都在 `finally` 中恢复原时间与播放状态，且只在采集前循环运行时重新启动。聚焦测试覆盖暂停非零时间、播放中以及原本未运行循环不被启动；验收脚本消费 analyzer 完整 JSON，而不是只依赖精简 stdout，避免把预期指标失败误判为 `analysisFailures`。本阶段只迁移 BaseColor，不迁移视角相关高光/各向异性/ToonRamp/ShaderToRGB，也不修改 PMX、VMD、骨骼、权重、Morph、IK、Grant、Physics、拓扑或材质槽。证据位于 `web/.scratch/reze-k3-v1-stage/`、`visual-diff.json`、`visual-diff-swap-slot-target.json`、`visual-diff-wrongtint.json` 与交付报告。
