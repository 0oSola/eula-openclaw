...
已收口 HairA/HairB 正式 Gate 契约，不改变 HairA/HairB 视觉公式。`V14D_HAIR_AUTHORITATIVE_CAPTURE` 固定为 `currentSeconds=4`、`currentFrame=120`、`fps=30`、`animationName=koleda-v14d-authoritative-pose-f120.vmd`。`validateV14dHairCapturePair` 逐侧验证 pixel↔triUV 原子证据，再执行绝对秒数、帧号、帧率（若证据携带）和非空精确动画名校验；双方同时 frame0、同时错误秒数或同时空/错名不会因彼此相等而通过。

G3 报告现在记录 requested/actual seconds、frame、fps、animationName、original/V1 captureId 和 pixel↔triUV pair。accept 另外保存 `g3-hair-original-atomic.json` 与 V1 原子摘要，包含 `captureEvidence`、`captureState`、采集前后进度和 HairA/HairB 材质样本/解析计数，不复制 analyzer 不需要的 original 大型 triUV 数组。
...
- `web/src/features/stage/v14dHairCaptureState.js`：新增固定权威采集常量与绝对 pair 校验。
...
- `web/tests/v14d-hair-partition.test.mjs`：先红后绿覆盖健康固定帧、双方 frame0、双方错误秒数、双方空/错名及 captureId/时间错配，并检查 analyzer 默认路径。
...
- `node --test tests/v14d-hair-partition.test.mjs`：exit=0，20/20，通过后无跳过项。
...
- `node scripts/accept-reze-k3-v1-stage.mjs`：exit=0；全新 `127.0.0.1:3114` 的 /companion G1-G6 全部 pass，服务随后停止。
...
`gate-report.json` 实际审计字段：requested 与 original/V1 actual 均为 4 秒、120 帧、30 FPS、`koleda-v14d-authoritative-pose-f120.vmd`；original captureId=1、V1 captureId=2，双方 pixel↔triUV pair 均 `ok=true`。original 原子摘要的材质键为 HairA/HairB，并含 `captureEvidence`、`captureState` 和解析计数。
...
- 未运行全量 Node 测试套件；20/20 为票据聚焦测试，不能外推为全量测试通过。
