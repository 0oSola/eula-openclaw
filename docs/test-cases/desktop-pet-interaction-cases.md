...
| PET-INT-037 | Codex 启动 | 右键 `New Codex Session` | 主进程写入 VSCode terminal request，并用 helper extension 在 VSCode 内置终端 `Codex Pet` 执行 `codex`，不启动外部 `cmd.exe` | 已覆盖 | `desktop-pet/electron/codexLauncher.test.ts`, `desktop-pet/vscode-helper/extensionCore.test.ts` |
...
| PET-INT-048 | VSCode helper | `MMD_PET_VSCODE_HELPER_MODE=installed` | Pet 写入 terminal request 后只执行 `code --reuse-window <workspace>`，不传 `--extensionDevelopmentPath`，由已安装 helper extension 处理 request | 已覆盖 | `desktop-pet/electron/codexLauncher.test.ts` |
| PET-INT-049 | VSCode helper | 未设置或设置未知 helper mode | Pet 回退 development 模式，继续传 `--extensionDevelopmentPath <desktop-pet/vscode-helper>`，保持当前开发运行方式 | 已覆盖 | `desktop-pet/electron/codexLauncher.test.ts` |
...
| PET-INT-051 | Prompt 发送 | 在 prompt 面板发送非空文本 | preload 通过 `pet:prompt:send` 请求主进程；主进程写入 `mode=prompt` terminal request，并用 VSCode helper 发送到 `Codex Pet` 内置终端 | 已覆盖 | `desktop-pet/electron/codexLauncher.test.ts`, `desktop-pet/electron/preloadModuleFormat.test.ts` |
